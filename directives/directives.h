...
int is_identifier(char* str) {
	...
	res1 = (int)(str[0] - 'a');
	res2 = (int)(str[0] - 'A');
	if (str[0] != '_' && (res1 < 0 || res1 > 25) && (res2 < 0 || res2 > 25)) {
		return 0;
	}
...
int define_directive(FILE* f_in, FILE* f_out, char words[][WORD_SIZE], int nr_words) {
	char* symbol;
	char* value;
	int len = 1;
	...
	if (!symbol) {
		printf("Error: Can not alloc memory for \'symbol\' variable.\n");
		return ENOMEM;
	}
	...
	if (nr_words >= 3) {
		is_multiline = 0;
		result = define_helper(word, words, nr_words, &len, 2, &is_multiline);
		if (result != SUCCESS) {
			free(symbol);
			return result;
		}
		...
		if (is_multiline) {
			...
			while (fgets(line, LINE_SIZE, f_in) != NULL) {
				...
				split_line(line, words_def, &nr_words_def, delm);
				if (nr_words_def == 0) {
					fprintf(f_out, "\n");
					break;
				}

				result = define_helper(word, words_def, nr_words_def, &len, 0, &is_multiline);
				if (result != SUCCESS) {
					free(symbol);
					free(line);
					return result;
